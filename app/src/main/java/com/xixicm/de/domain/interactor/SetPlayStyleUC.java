...
/**
 * A use case to set play style setting.
 *
 * @author mc
 */
public class SetPlayStyleUC extends AbstractUseCase<Integer, Void, Void> {
    PreferenceRepository mPreferenceRepository;

    public SetPlayStyleUC(PreferenceRepository preferences) {
        mPreferenceRepository = preferences;
    }

    private void setPlayStyle() {
        LogUtils.v(Constants.TAG, "set play style to " + getRequestValue() + ".");
        mPreferenceRepository.setPlayStyle(getRequestValue());
    }
