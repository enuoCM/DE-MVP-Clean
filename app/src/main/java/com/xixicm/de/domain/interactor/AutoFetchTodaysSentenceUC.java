...
/**
 * A auto trigger use case to fetch today's sentence.  No request parameter, No response,
 * implementation should base on other notification scheme, such as EventBus
 *
 * @author mc
 */
public class AutoFetchTodaysSentenceUC extends AbstractUseCase<Void, Void, Void> {
    SentenceFetchExecutor mSentenceFetchExecutor;

    public AutoFetchTodaysSentenceUC(SentenceFetchExecutor sentenceFetchExecutor) {
        mSentenceFetchExecutor = sentenceFetchExecutor;
    }

    private void fetchTodaysSentence() {
        LogUtils.v(Constants.TAG, "auto start to fetch today's sentence.");
        mSentenceFetchExecutor.fetchTodaysSentence(false);
    }
